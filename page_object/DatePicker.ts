...
export class DatePicker {
  ...
  async getMonth(month: string) {
    await this.page.selectOption(this.monthOfTheYear, { label: month });
  }
  ...
  randomYearNumber = Math.floor(Math.random() * 10) + 1;
  randomYearNumber2 = _.random(1, 50); //lodash
  date = _.random(1, 30); //lodash
  async dateFromToday() {
    let obj = {
        Jan:"01",
        Feb:"02",
